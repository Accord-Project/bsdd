...
:PROPERTIES:
:CUSTOM_ID: refactored-bsdd-schema-plantuml-overview-diagram
:END:

[[https://github.com/VladimirAlexiev/soml/tree/master/soml2puml][soml2puml]] is a new open source tool that uses PlantUML to generate diagrams from SOML schemas.
...
#+label: fig:bsdd-graphql-diagram-overview
#+caption: Refactored bSDD Schema: PlantUML Overview Diagram
[[./img/bsdd-graphql-soml-diagram-overview.png]]

** Refactored bSDD Schema: PlantUML Full Diagram
:PROPERTIES:
:CUSTOM_ID: refactored-bsdd-schema-plantuml-full-diagram
:END:

The full schema uses the same layout, but also adds attributes:

#+label: fig:bsdd-graphql-diagram-full
#+caption: Refactored bSDD Schema: PlantUML Full Diagram
...
- ~graphql~: GraphQL queries to get the 9 kinds of entities from the original GraphQL endpoint
  - Also ~schemaIntrospection.graphql~ that gets the GraphQL schema of an endpoint
    (you can find more at the [[https://bsdd.ontotext.com/graphiql/][Refactored GraphiQL Endpoint]])
...
:PROPERTIES:
:CUSTOM_ID: time-dependent-search-uris
:END:
Testing URIs above, we had noticed that search results for the same queries is time-dependent. 
E.g., search for the term "CableSegment" using the interactive [[https://search.bsdd.buildingsmart.org][bSDD Search]] UI returns in May 2023 IfcCableSegment with URI
https://search.bsdd.buildingsmart.org/Classification/Index/70992, 
...
- In some cases, the ~<org>~ name doesn't quite mesh with the domain name, perhaps due to the way bSDD allocates ~<org>~ identifiers to bSDD contributors
  ...
  - "digibase/volkerwesselsbv": [[https://www.bimregister.nl/actueel/video/author/89-delanokenepa?start=250][bimregister.nl news from 2018]] suggest that ~digibase~ is a new company/initiative within Volker Wessel
...
:PROPERTIES:
:CUSTOM_ID: self-describing-langstrings
:END:
...
:PROPERTIES:
:CUSTOM_ID: use-language-content-negotiation
:END:
...
- ~Property.{allowedValues,possibleValues}~: ~possibleValues~ is deprecated and should be removed. UPD (19 May 2023): bSI [[took our suggestion][https://github.com/buildingSMART/bSDD/commit/f72e627a45540716a06168eb6292ce3005ee4911]] and removed deprecated field ~possibleValues~.
...
UPD (19 May 2023): bSI [[took our suggestion][https://github.com/buildingSMART/bSDD/commit/f72e627a45540716a06168eb6292ce3005ee4911]] and removed deprecated field ~possibleValues~.
...
We wrote the SPARQL Update script [[./scripts/transform.ru][transform.ru]] (~.ru~ is the W3C standard extension for this type of file).
It does the following:
...
:END:
[[https://bsdd.ontotext.com/graphdb/graphs-visualizations?config=b977ae2529094192b469f3484029fc48][This visualization]] shows classifications that have relations (we ignore the relation name).
...
and provided a SPARQL endpoint using [[https://graphdb.ontotext.com/][Ontotext GraphDB]], and GraphQL endpoint using [[https://platform.ontotext.com/semantic-objects/][Ontotext Platform Semantic Objects]].
...
- Take the draft bSDD ontology [[https://github.com/buildingSMART/bSDD/blob/master/RDF/preview-bsdd-rdfs-0.4.ttl][preview-bsdd-rdfs-0.4.ttl]] (May 2022) and improve it.
