...
We conduct a thorough comparison and analysis of bSDD data related to fulfillment of FAIR (findable, accessible, interoperable, and reusable) principles by bSDD. 
Based on this analysis, we propose a set of enhancements to make bSDD data better structured and more FAIR.
We implement many of the suggestions and provide GraphQL and SPARQL endpoints where bSDD data is better structured and more "semantic".
Our detailed work is available at [[https://github.com/Accord-Project/bsdd]] (source) and [[https://bsdd.ontotext.com]] (data and sample queries).
...
:CUSTOM_ID: bsdd
:END:
Reusable data dictionaries serving interoperation purposes are widely known in the industry, for example, IEC Common Data Dictionary (IEC CDD)  and eCl@ss - for the electronic exchange of digital information across industries, ISO 15926 part 4 Reference Data Libraries [[https://rds.posccaesar.org/]] - for the electronic exchange of digital information across process plant industries. buildingSMART Data Dictionary (bSDD) is the solution for the AECO domain.

The bSDD is a multilingual hierarchically structured encyclopedia of object concepts and their properties used in building information models (BIM). Predefined by some vendors sets of properties extending common property sets of Industry Foundation Classes (IFC) allow to describe specific domains (e.g. transportation) or specific building elements (e.g. doors, windows, stairs). bSDD is
organized according to the Building Information Modelling methodology defined in ISO 23386 (2020) following the language-independent information model used for the development of dictionaries storing information about construction work defined in ISO 12006-3 (2022) . 

bSDD was initiated to support improved interoperability in the building and construction industry.
bSDD is a very comprehensive solution aiming at the provisioning of open complementary product data definitions, identification, and distribution methods.
...
:END:
GraphQL [[https://graphql.com]] as a query language for APIs and a runtime for fulfilling those queries with the existing data has many benefits over traditional REST APIs: 
ability to avoid over fetching by specifying exactly which data and in which structure is to be returned from the server; 
ability to retrieve many resources in a single request; GraphQL service defines a set of types which completely describe the set of possible data queriable with that service. 
bSDD makes use of ~GraphiQL~ - a graphical interactive in-browser GraphQL IDE, available at [[https://test.bsdd.buildingsmart.org/graphiql/]]. 
...
:END:
We investigated the original bSDD schema and datasets available at [[https://test.bsdd.buildingsmart.org]] with [[https://rawgit2.com/Accord-Project/bsdd/main/bsdd-graphql-voyager-orig.html][GraphQL Voyager]]. 
In Figure 1 the original schema is depicted in class and class+fields views.
...
It can invoke given query with or without variables and returns the response to the user in JSON. To work with the script we need to create a virtual Python environment:
...
[[bsdd-graphql-voyager-orig.html]] - visualization of the original schema
...
Is there a need for two different web pages showing nearly the same info?
...
Otherwise, there is a danger that people will start copying these non-semantic URLs in communications and AECO data.
...
:END:
Querying all domains with all their classifications results in a GraphQL error.
It is due to ~classificationSearch~ being marked as non-nullable, whereas the backend API is returning ~null~.
...
:END:
The GraphQL API returns domains that are not available independently. One such domain is ~http://identifier.buildingsmart.org/uri/spr/spr-cfhios-0.1~.
...
The second response for ~domain~ will be ~null~ although the domain is present in the ~domains~ response, so the situation is similar to the previous one.
...
There are classifications for which some of the properties do not return, which results in ~NULL_REFERENCE~ errors, although the fields are nullable.
#+begin_src graphql
 ...
 #+end_src
This query will return the classification along with four out of five properties, and the fifth property will be ~null~.
...
The classification properties have a ~predefinedValue~ field, but it is of type ~String~ instead of utilizing ~ClassificationPropertyValue~.
...
Every classification property has a scope within the owning classification. Therefore it may include data applicable only within that particular classification.
Classification properties include a ~namespaceUri~ field, a global property identifier, not the one within the classification. Therefore, classification properties cannot be uniquely identified and located in the scope of the particular classification.
...
The same situation occurs with JSON API response: the properties have no unique identifier. However, the RDF TTL representation is the opposite.Requesting ~text/turtle~ from the JSON API will return the following part:
...
We see that ~ACResistance~ has a scope within ~IfcCableSegmentCABLESEGMENT~ classification and is easily identified and located.
...
#+end_src
According to [[hierarchical URIs design][https://patterns.dataincubator.org/book/hierarchical-uris.html]]], domain URL can be more "hackable", allowing users to navigate the hierarchy by pruning the URI. The proposed URL structure can be 
...
This data type is like a ~String~, but it is used for identifiers.
...
Properties and ClassificationProperties have most fields the same and differ in five ones: ~connectedPropertyCodes~ [String] and ~relations~ [PropertyRelation] belong uniquely to ~Property~, ~isRequired~ (Boolean), ~isWritable~ (Boolean), ~predefinedValue~ (String), ~propertySet~ (String) and ~symbol~ (String) - to ~ClassificationProperty~. PropertyValues and ClassificationPropertyValues are essentially the same by the current modeling decision.
...
- More importantly, these key values are not used:
...
~connectedProperty~ is used only on seven Properties (and not Classification Properties):
...
|135250	|false|
|12385	|true |
Additionally, ~dynamicParameterPropertyCodes~ ranges in ~String~, however, it is more realistic to have ~[String]~ - an array of strings to describe relevant compounds of such dynamic properties.
...
- There are several useless types (~Country, Language, ReferenceDocument, Unit~).
  Instead of relations pointing to these types, the other types have properties  (e. g. ~countryOfOrigin~, ~countriesOfUse~) representing the same information ranging in ~String~.
...
- ~countriesOfUse~: array of country code strings. Should become an array of relations to ~Country~,
- ~documentReference~: a string, and it's unclear whether it should be the URL of a ~ReferenceDocument~, a bibliographic reference, a title, or some other free text. Should become relation to ~ReferenceDocument~.
- ~relatedIfcEntityNames~: since IFC is present as a bSDD Domain, ~relatedIfcEntityNames~ could be converted to a relation to the respective IFC Classification.
...
:END:
The following types are very similar, and most of their fields duplicate between them, with no modularity or inheritance:
...
But since there are no rules on which fields of ~Property~ to reuse in  ~ClassificationProperty~, the latter type copies most of the fields from the former.
...
The problem is that all these Classification Properties copy the same field values from the Property, over and over again:
#+begin_src sparql
PREFIX bsdd: <http://bsdd.buildingsmart.org/def#>
...
A GraphQL schema can define mandatory/optional status at the level of array or the level of individual elements:
...
(It means that there is no way to enforce a *non-empty* array in GraphQL.)

bSDD specifies arrays like [Classification], which is the most permissive specification. It means that [null, null, null] is a valid answer to a query that returns an array of Classifications. However, such an answer is not suitable because null elements are useless.
...
E. g. let's take the Turtle (RDF) fragment already used in the previous sections:
#+begin_src turtle
<https://identifier.buildingsmart.org/uri/buildingsmart/ifc-4.3/class/IfcCableSegmentCABLESEGMENT/ACResistance>
  bsdd:ClassificationProperty <https://identifier.buildingsmart.org/uri/buildingsmart/ifc-4.3/class/IfcCableSegmentCABLESEGMENT>;
  bsdd:PropertyDomainName "IFC";
...
- Property names should start in lowercase
...
:PROPERTIES:
:CUSTOM_ID: no-rules-how-to-express-lack-of-data
:END:

There are no rules on how to express the lack of ~physicalQuantity~ for dimensionless properties (eg count, percentage) or enumerated properties (having a list of ~allowedValues~).
For example:
...
:END:
We detected the following types can be converted into first-class types.
- PropertySet
- DomainVersion
- CountrySubdivision
...
:END:
There is a field ~possibleValues~ that is presented in the GraphQL as "deprecated", but still it is seen in the GraphQL schema.
...
- the logical ordering of values through ~sortNumber~ (as opposed to always alphabetical ordering)
...
- ~predefinedValue~ needs to hold not just enumeration values but also Real, String, Boolean, etc.
  However, next, it should be structured as a variant and not be cast down into a String.
...
:END:
Currently there is no way to search arbitrary type or field value with bSDD GraphQL, the only exception is the search of classifications within a domain with ~classificationSearch~ query parameter. However, bSDD GraphQL users will benefit from e. g. querying of compound fields, with their constituents, of dynamic fields (although currently none has constituents), classifications with relations, and their related classifications, properties with relations, and their related properties.
...
:END:
An example classification in JSON obtained with the following command (shortened for brevity):
