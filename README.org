...
** Modeling Issues
Following the previous section, all significant classes should have ~ID~,
(which, in the case of linked data, is an URL).
In addition to pure technical recommendations to ease the findability and accessibility of data in bSDD by improving their URI patterns, we have noticed several modeling issues.
...
with the name "Pile location" and the definition "Gather properties to locate a pile."
...
and this is wrong approach, because e. g. one cannot find all Thickness properties easily.
...
HTML Entities are not resolved in some ~bsdd:description~. Instead of real chars, they are represented as HTML encodings.
