...
:CUSTOM_ID: bsdd
:END:
The buildingSMART Data Dictionary (bSDD) [@buildingsmartinternationalBuildingSMARTDataDictionary] is a vocabulary
structured according to ISO 12006-3 (2022) [@ISO1200632022] and ISO 23386 (2020) [@ISO233862020]
that TODO.
Basis for Interoperability TODO
TODO currently consists of some 80,000 concepts with translations in multiple languages"

@palosStateoftheartAnalysisProduct2012 "International Framework for Dictionaries (IFD) aka buildingSMART Data Dictionary (bSDD) is a very comprehensive solution, which aims to provide open complementary product data definitions, identification and distribution methods. The bSDD is a unified reference library of terms and properties, intended to support improved interoperability in the building and construction industry. The bSDD will be an open, shared, international terminology library for structuring object-oriented information exchange. ThebSDDisanobject-orienteddatabase,programmed with EXPRESS data modeling language. The library classifies and defines properties for structured identification of commonly used terms and concepts. The system assigns a Global Unique Identifier (GUID) for all defined terms entered into the system in order to enable data utilization in variety of structured documents and applications.

@erikjonssonConsequencesImplementingBuildingSMART2015 "bSDD is an international standard developed by the neutral organization buildingSMART. It is a standard that defines the terminology surrounding BIM objects. Much research has been done on the potential benefits of BIM in general.
The purpose of this thesis is to determine how a property development and construction company, that already works with BIM in many of its projects, would be affected by an implementation of the bsDD. The possible consequences of bsDD implementation are mapped out and analyzed through a literature review. In addition to this, a simulation is conducted with the purpose of showing how object libraries can be created on an organizational level.

Reusable data dictionaries serving interoperation purposes are widely used in industry, for example, eClass[[]], ISO 15926 part 4 Reference Data Libraries [[https://rds.posccaesar.org/]], TODO more.    
* Related Work
:PROPERTIES:
...
@beetzInteroperableDataModels2014 "bSDD vocabulary has been transformed into a configurable RDF dataset. On the meta-model level a number of different modeling approaches ranging from OWL to RDFS and SKOS have been implemented to evaluate the advantages and disadvantages of the respective modeling strategies"

@oraskariLiveWebOntology2021
...
@kebedeIntegrationManufacturersProduct2022 "bSDD is a service deployed to facilitate a common language in the built environment while allowing the creation of multilingual dictionaries to enable designers to enrich their model and extend IFC.
The Building Product Ontology (BPO) was developed as a core ontology for linked building product data to describe real product structures and properties. BPO uses bSDD to allow users to semantically enrich product descriptions by referencing the terms using bSDD URLs, selected as it comprises a wide range of terms employed in the AEC industry. This work shows where and how the bsDD can fit in the LBD ecosystem. The new development of bSDD, which embraces a shift toward publishing data classifications and properties as Linked Data, indicates a substantial step forward for the industry to take advantage of these technologies [55].
...
:END:
We investigated original bSDD schema and datasets available at [[https://test.bsdd.buildingsmart.org]] with [[https://rawgit2.com/Accord-Project/bsdd/main/bsdd-graphql-voyager-orig.html][GraphQL Voyager]]. In the Figure 1 the original schema is depicted in class and class+properties views.
