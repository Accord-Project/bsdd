...
The repository includes a Python script ~bsdd2csv.py~ that can connect to the bSDD GraphQL API and export all objects in the
GraphQL schema as CSV including domains, classifications, classification properties etc.

The exported data will be located under the ~data/~ directory once the script is executed. Running this script takes around ~8 hours due to
the large quantity of classifications and classification properties in the GraphQL API as well as the slow query responses from the API.

The overall process is of the script is

- Export all "root" types with their complete information except relations (object properties)
  - bSDD domains in ~data/domains.csv~ (~Domain~ in GraphQL)
  - Countries in ~data/domains.csv~ (~Country~ in GraphQL)
  - Languages in ~data/languages.csv~ (~Language~ in GraphQL)
  - Measurements units in ~data/units.csv~ (~Unit~ in GraphQL)
  - Reference documents in ~data/reference_documents.csv~ (~ReferenceDocument~ in GraphQL)
- Export a mapping between domains and their classifications in ~domain_classifications.csv~, this steps collects all domain identifies from
  ~data/domains.csv~ and queries their classifications
- Export all classifications based on the mapping between domains in ~classifications.csv~, this steps collects classifications identifiers
  from ~domain_classifications.csv~ and queries their complete information except relations (object properties)
- Export all classifications' properties, relations and child classifications, this step collects classifications identifiers
  from ~domain_classifications.csv~ and collects only the object properties for classification. However, it queries the full properties
  information. The steps produces the following files
  - Classification to classification property mapping and classification property information in ~data/classification_properties.csv~
  - Classification to relations mapping in ~data/classification_relations.csv~
  - Classification to child classifications mapping in ~data/classification_children.csv~
