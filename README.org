...
:END:
Currently there is no way to search arbitrary type or field value with GraphQL, the only exception is the search of classifications within a domain with ~classificationSearch~.
