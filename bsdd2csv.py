...
def query_graphql(endpoint, query):
...
def get_domains():
    return query_graphql(GRAPHQL_API, """
      query getDomains {
        domains {
          id: namespaceUri
          name
...
def get_domain_classifications(domain):
    result = query_graphql(GRAPHQL_API, """
      query getDomainClassifications {
        domain(namespaceUri: "$DOMAIN") {
    ...
    logger.warning(f"Domain {domain} has no result or classifications!")
    return list()
...
def get_classification(classification):
    return query_graphql(GRAPHQL_API, """
        query getClassification {
          classification(namespaceUri: "$CLASSIFICATION") {
            id: namespaceUri
            classificationType
...
def get_classification_object_properties(classification_id):
    return query_graphql(GRAPHQL_API, """
    ...
    """.replace("$CLASSIFICATION", classification_id))["classification"]


def get_global_property(property_id):
    return query_graphql(GRAPHQL_API, """
        query getProperty {
          property(namespaceUri: "$PROPERTY") {
            id: namespaceUri
            allowedValues {
              id: namespaceUri
              code
    ...
    """.replace("$PROPERTY", property_id))["property"]
...
def export_root_data():
    ...
    reference_documents = get_reference_documents()
...
def export_classification_object_properties():
    ...
    for index, classification_id in enumerate(classifications):
        try:
            classification = get_classification_object_properties(classification_id)
...
def export_global_properties():
    ...
    for index, classification_property in enumerate(classification_properties):
        try:
            global_property = get_global_property(classification_property)
